...
describe('applyPlanEdit', () => {
  ...
  it('returns not_pending for non-pending entry', () => {
    enqueue('task-1', makePlan());
  ...
  it('returns not_found for unknown entry', () => {
    const result = applyPlanEdit({ entryId: 'ghost', stepUpdates: [] });
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('not_found');
  });

  it('skips metadata update when no changes applied', () => {
    enqueue('task-1', makePlan());
    const before = getEntry('task-1')!;
    const result = applyPlanEdit({
      entryId: 'task-1',
      stepUpdates: [{ stepId: 'step-1', description: 'Create index.ts' }], // same value
    });
    expect(result.ok).toBe(true);
    if (result.ok) {
...
describe('recordDecision', () => {
  it('marks entry as approved', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.entry.status).toBe('approved');
  });
  ...
  it('marks entry as revision_requested', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.entry.status).toBe('revision_requested');
  });
